...
router.get('/', asyncHandler(async (req, res) => {
  const { rows } = await query(
    `SELECT id, name, email, role, initials, active, excluded_from_chore_rotation,
            show_parts_on_dashboard, dashboard_priority_personal_only, created_at
...
router.patch('/:id', requireAdmin, asyncHandler(async (req, res) => {
  const {
    ...
    // Settings -> Staff accounts' "Priority & To-Do's" scope checkbox
    // (migration 059) -- same shape again: off (shop-wide) by default,
    // an admin opts a specific person into seeing just their own slice.
    dashboard_priority_personal_only: dashboardPriorityPersonalOnly,
  } = req.body || {};
  ...
  const { rows } = await query(
    `UPDATE employees SET
        ...
        dashboard_priority_personal_only = COALESCE($12, dashboard_priority_personal_only)
      WHERE id = $1
    ...
    [
      ...
      dashboardPriorityPersonalOnly === undefined ? null : dashboardPriorityPersonalOnly,
    ],
  );
