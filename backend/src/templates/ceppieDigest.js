...
/**
 * Builds the weekly Ceppies digest email — services/ceppies.js's
 * sendCeppieDigest(), fired either by the schedule (services/
 * ceppieScheduler.js) or an admin's manual "Send now" (routes/ceppies.js).
...
/** `nominations` is an array of ceppie_nominations rows joined to nominee_name/nominator_name. */
function buildCeppieDigestEmail({ nominations }) {
  const body = nominations.length
    ? nominations.map(nominationRow).join('')
    : `<p style="margin:0;font-size:14px;color:#6b7280;line-height:1.5;">
         No Ceppie nominations came in this round — nominate a teammate any time from the
         Ceppies tab.
       </p>`;
...
<div style="background:#f4f4f5;padding:32px 16px;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;">
  <div style="max-width:560px;margin:0 auto;background:#ffffff;border-radius:10px;overflow:hidden;border:1px solid #e5e5e5;">
    ...
    <div style="padding:28px;">
      <h1 style="font-size:20px;margin:0 0 4px;color:#16181d;">This week's Ceppie nominations</h1>
      ...
      <p style="margin:20px 0 0;font-size:12px;color:#9aa1ad;text-align:center;">
        Nominate a teammate any time from the Ceppies tab in Mission Control.
      </p>
    </div>
  ...
  return {
    subject: "This week's Ceppie nominations — Chicago Electric Piano Company",
    html,
    attachments: [
      {
...
module.exports = { buildCeppieDigestEmail };
